#!/usr/bin/env sh
# SessionStart hook: injects the full adhd ruleset when the user has
# opted in by creating $CLAUDE_CONFIG_DIR/.adhd-always (default ~/.claude).
...
flag_path="$claude_dir/.adhd-always"
...
skill_path="$script_dir/../skills/adhd/SKILL.md"
[ -f "$skill_path" ] || exit 0
